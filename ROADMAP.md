...
| Reader controller explores from the static start every run; the settled count is not persisted to the profile | — | Medium | Open (v0.15.44) — costs ~1.5 s of climbing per run on a 32 GiB input; persisting it per (machine, direction) would make run 2 start settled |
| `--adapt` reader controller is compress-only; `-d --tar` extract and the decompress prefetch reader still use the one-shot 2x scale-up latch | tar | Medium | Open (v0.15.44) |
