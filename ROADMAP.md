...
| `--adapt` does nothing for operations shorter than `RAMP_SEC` | — | Medium | Open (found 2026-08-04) — the governor classifies nothing for the first 3.0 s, so any run finishing inside ~4 s never leaves warmup and gets no adaptation at all. Measured: extract from a healthy drive completed its pipeline in 2.59 s and produced no `[ADAPT] regime shares` line whatsoever. **This gets WORSE as hardware gets faster** — more operations finish inside the ramp — so it will bite the 8xH100/256-core box harder than this one. Interacts with the v0.15.55 evidence gate (2 s / 25% classified) which correctly refuses to persist a verdict from such a run, so fast runs also teach the profile nothing. Wants the ramp derived from observed signal stability rather than a fixed wall-clock constant; do NOT simply shrink the constant, which trades one arbitrary number for another |
