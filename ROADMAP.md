...
| `--adapt` reader controller not wired into `--tar` create | tar | Medium | Open (v0.15.46) — TRIED and reverted: readers claim a chunk then wait for the pusher's window, so parking strands work the pusher waits on. Deadlocked a 20 GiB tree at 17.6 GB in, 116 threads in futex_wait; fine without `--adapt`. Needs the reorder-buffer invariants worked out first |
