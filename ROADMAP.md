...
| `--adapt` governor classifies clearly I/O-bound runs as COMPUTE_BOUND | — | Medium | Open (found 2026-08-04, **two independent instances**) — (1) cold 32 GiB read: `[READER] io 105.7%, blocked-downstream 0.0%, verdict "reader saturated — the device is the faucet"`, while `[ADAPT]` recorded `source-bound 1.1 s (3%) compute-bound 34.1 s (89%)`; the documented rule is `rbusy >= SOURCE_BUSY(0.85) && rblocked <= 0.05`, and 1.057/0.0 satisfies it for essentially the whole run. (2) compress to a saturated sink: `[WRITER] verdict: output device saturated` while the regime recorded compute-bound. Both disagreements point the SAME way — the governor under-reports I/O-bound. Warm runs classify correctly (io 65.6%, blocked 18.8% → compute-bound), so it is specific to the saturated case. Suspect the governor's window deltas are not fed by the same counters the `[READER]`/`[WRITER]` verdicts use (there is a known precedent: the mmap reader leaves all four reader counters at zero). **This is upstream of the v0.15.57 gate and limits its reach**: on a box where a genuinely reader-bound run is misclassified, the reader controller now correctly stands down but also cannot help |
